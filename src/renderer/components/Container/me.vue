...
export default {
  ...
  data () {
    return {
      viewStack: [{component: 'profile', args: {id: 'mogita'}, belongsTo: 'me', nonce: 0, title: 'mogita'}],
      direction: ''
    }
  },
  methods: {
  ...
  mounted () {
    this.$bus.$off('timeline.me.view.push')
    this.$bus.$on('timeline.me.view.push', args => {
      this.direction = 'forward'
      this.viewStack.unshift(args)
    })
    this.$bus.$off('timeline.me.view.pop')
    this.$bus.$on('timeline.me.view.pop', this.handleGoBack)
  }
}
</script>
